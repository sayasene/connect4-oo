...
class Game {
  ...
  makeBoard() {
    console.log("makeBoard runs")
    for (let y = 0; y < this.HEIGHT; y++) {
      this.inMemoryBoard.push(Array.from({ length: this.WIDTH }));
    }
  }

  makeHtmlBoard() {
    console.log("makeHtmlBoard runs")
  ...
  findSpotForCol(x) {
    console.log("findSpotForCol runs")
    for (let y = this.HEIGHT - 1; y >= 0; y--) {
      if (!this.inMemoryBoard[y][x]) {
        return y;
      }
    }
    return null;
